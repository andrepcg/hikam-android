...
public final class zzarc {
    ...
    public int cF() throws IOException {
        ...
        int i = cM & TransportMediator.KEYCODE_MEDIA_PAUSE;
        byte cM2 = cM();
        if (cM2 >= (byte) 0) {
            return i | (cM2 << 7);
        }
        i |= (cM2 & TransportMediator.KEYCODE_MEDIA_PAUSE) << 7;
        cM2 = cM();
        if (cM2 >= (byte) 0) {
            return i | (cM2 << 14);
        }
        i |= (cM2 & TransportMediator.KEYCODE_MEDIA_PAUSE) << 14;
        cM2 = cM();
        if (cM2 >= (byte) 0) {
            return i | (cM2 << 21);
        }
        i |= (cM2 & TransportMediator.KEYCODE_MEDIA_PAUSE) << 21;
        cM2 = cM();
        i |= cM2 << 28;
        if (cM2 >= (byte) 0) {
            return i;
    ...
    public long cG() throws IOException {
        ...
        for (int i = 0; i < 64; i += 7) {
            byte cM = cM();
            j |= ((long) (cM & TransportMediator.KEYCODE_MEDIA_PAUSE)) << i;
            if ((cM & 128) == 0) {
                return j;
            }
        }
