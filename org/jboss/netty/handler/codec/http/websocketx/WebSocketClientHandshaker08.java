...
public class WebSocketClientHandshaker08 extends WebSocketClientHandshaker {
    ...
    public ChannelFuture handshake(Channel channel) throws Exception {
        ...
        request.headers().add(Names.SEC_WEBSOCKET_VERSION, (Object) "8");
        ...
        channel.write(request).addListener(new ChannelFutureListener() {
            public void operationComplete(ChannelFuture future) {
                future.getChannel().getPipeline().replace(HttpRequestEncoder.class, "ws-encoder", new WebSocket08FrameEncoder(true));
                if (future.isSuccess()) {
    ...
    public void finishHandshake(Channel channel, HttpResponse response) {
        if (response.getStatus().equals(HttpResponseStatus.SWITCHING_PROTOCOLS)) {
            ...
            WebSocketClientHandshaker.replaceDecoder(channel, new WebSocket08FrameDecoder(false, this.allowExtensions, getMaxFramePayloadLength()));
            return;
        }
